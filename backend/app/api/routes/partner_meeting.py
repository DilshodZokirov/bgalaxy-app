...
# Tracks who is allowed into each ad-hoc partner room: room_name -> set of
# user_ids (the starter + everyone they invited). In-memory is fine here —
# same pattern as the 1-1 call state in meetings.py.
_partner_rooms: dict[str, set[str]] = {}
...
async def start_partner_meeting(
    ...
    _partner_rooms[room_name] = {str(current_user.id)} | {str(p.id) for p in partners}
...
async def join_partner_meeting(
    ...
    allowed = _partner_rooms.get(room_name or "", set())
...
async def add_to_partner_meeting(
    ...
    allowed = _partner_rooms.get(room_name)
