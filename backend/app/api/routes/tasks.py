...
def _to_out(task: Task, names: dict) -> TaskOut:
    return TaskOut(
        id=task.id,
        title=task.title,
...
async def list_tasks(
    ...
    names = await _names_map(db, {str(t.assignee_id) for t in tasks} | {str(t.created_by) for t in tasks})
    return [_to_out(t, names) for t in tasks]
...
async def create_task(
    ...
    names = await _names_map(db, {str(t.assignee_id) for t in created_tasks} | {str(current_user.id)})
    return [_to_out(t, names) for t in created_tasks]
...
async def update_task(
    ...
    names = await _names_map(db, {str(task.assignee_id), str(task.created_by), str(task.checked_by or "")})
    return _to_out(task, names)
...
async def task_history(
    ...
    items = [_to_out(t, names) for t in tasks]
